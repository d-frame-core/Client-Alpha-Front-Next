...
const [surveyDeletedToaster, setSurveyDeletedToaster] = useState(false);
  ...
  async function getAllCampaigns() {
    ...
    await axios
      .get(`https://client-backend-402017.el.r.appspot.com/survey/client/${id}`, {
        headers: {
  ...
  async function deleteParticularSurvey(surveyId:any) {
    ...
    const res = await axios.delete(`https://client-backend-402017.el.r.appspot.com/survey/deleteSurvey/${surveyId}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
    ...
    <div>
            ...
            <Table sx={{ minWidth: 700 }} aria-label="customized table">
              <TableHead>
                <TableRow className='bg-red-300'>
                  <StyledTableCell className='border-2 border-blue-600 text-center' align='center' >Survey Name</StyledTableCell>
                  <StyledTableCell className='border-2 border-blue-600 text-center' align='center'>Status</StyledTableCell>
                  <StyledTableCell className='border-2 border-blue-600 text-center' align='center'>View</StyledTableCell>
                  <StyledTableCell className='border-2 border-blue-600 text-center' align='center'>Status</StyledTableCell>
                  <StyledTableCell className='border-2 border-blue-600 text-center' align='center'>totalReward</StyledTableCell>
                  ...
                  <StyledTableCell className='border-2 border-blue-600 text-center' align='center'>Delete</StyledTableCell>
                </TableRow>
              </TableHead>
              <TableBody>
                {
                allAdsDetails.map((row:any,index:number) => (
                  <StyledTableRow key={index}>
                    ...
                    <StyledTableCell align="right">{row.statusCampaign}</StyledTableCell>
                    ...
                    <StyledTableCell align="right">{row.totalReward}</StyledTableCell>
                    <StyledTableCell align="right">{row.startDate}</StyledTableCell>
                    <StyledTableCell align="right">{row.endDate}</StyledTableCell>
                    <StyledTableCell align="center" className='cursor-pointer' onClick={()=>deleteParticularSurvey(row._id)}><DeleteIcon className='text-red-500' /></StyledTableCell>
                  </StyledTableRow>
                ))}
              </TableBody>
            </Table>
      ...
      <Dialog open={open} onClose={handleClose}>
        ...
        <DialogContent>
          ...
          <p className='text-center text-[20px] text-gray-800'>{singleDetails.surveyDescription}</p>
          <div className='mt-4 m-auto w-[500px] text-[20px]'>
          <p className='w-[100%] my-2'><span className='text-gray-700 font-semibold mr-4'>Survey Status</span>: {singleDetails.statusCampaign}</p>
          ...
          <div >
            {
              singleDetails.totalQues?.map((singleQuestion:any,index:number)=>(
                <div key={index} className='bg-blue-300 rounded-lg mt-4'>
                  <p className='bg-blue-500 text-center py-2 rounded-t-lg'>Question : {singleQuestion.title}</p>
