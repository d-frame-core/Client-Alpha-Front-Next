...
export default function LearnMore() {
  ...
  const addLearnMore = async (title: string, text: string) => {
    await fetch(
      'https://client-backend-402017.el.r.appspot.com/Learnmore/addSingleLearn',
      {
        method: 'POST',
        headers: {
  ...
  const editLearnMore = async (id: string, title: string, text: string) => {
    await fetch(
      `https://client-backend-402017.el.r.appspot.com/Learnmore/admin/update/${id}`,
      {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
